jQuery(document).ready(function ($) {
  $(".atm-modern-player").each(function () {
    ...
    progressContainer.on("click", function (e) {
      if (audioElement.duration) {
        ...
        audioElement.currentTime =
          ((e.clientX - rect.left) / rect.width) * audioElement.duration;
      }
    });
    ...
    volumeBtn.on("click", function () {
      ...
      volumeBtn.html(
        `<span class="atm-volume-icon">${isMuted ? "🔇" : "🔊"}</span>`
      );
    });
    ...
    $(document).on("keydown", function (e) {
      if (player.is(":visible")) {
        ...
        if (e.which === 37 && audioElement.duration) {
          audioElement.currentTime = Math.max(0, audioElement.currentTime - 10);
        }
        if (e.which === 39 && audioElement.duration) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const tocLinks = document.querySelectorAll(".atm-toc-list a");
  const sections = document.querySelectorAll(
    ".atm-listicle-intro, .atm-listicle-item, .atm-listicle-conclusion"
  );
