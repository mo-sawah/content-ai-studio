...
// Helper functions (these should be defined in each component that needs them for now)
    ...
    return (
        <div className="atm-form-container">
            <div className="atm-grid-2">
                <TextControl label="Keyword" value={keyword} onChange={setKeyword} placeholder="e.g., AI in digital marketing" disabled={isLoading} />
                <TextControl label="or Article Title" value={title} onChange={setTitle} placeholder="e.g., 5 Ways AI is Revolutionizing Marketing" disabled={isLoading} />
            </div>
            <div className="atm-grid-3">
                <SelectControl label="Article Model" value={articleModel} options={modelOptions} onChange={setArticleModel} disabled={isLoading} />
                <SelectControl label="Writing Style" value={writingStyle} options={styleOptions} onChange={setWritingStyle} disabled={isLoading} />
                <SelectControl label="Article Length" value={wordCount} options={[ { label: 'Default', value: '' }, { label: 'Short (~500 words)', value: '500' }, { label: 'Standard (~800 words)', value: '800' }, { label: 'Medium (~1200 words)', value: '1200' }, { label: 'Long (~2000 words)', value: '2000' } ]} onChange={setWordCount} disabled={isLoading} />
            </div>
            <TextareaControl label="Custom Prompt (Optional)" value={customPrompt} onChange={setCustomPrompt} placeholder="Leave empty to use the selected Writing Style. If you write a prompt here, it will be used instead." rows="6" disabled={isLoading} />
            <CheckboxControl label="Also generate a featured image" checked={generateImage} onChange={setGenerateImage} disabled={isLoading} />
            <Button isPrimary onClick={handleGenerate} disabled={isLoading || (!keyword && !title)}>
                {isLoading ? <Spinner /> : 'Generate Creative Article'}
            </Button>
